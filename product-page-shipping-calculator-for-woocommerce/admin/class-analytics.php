...
class Pi_shipping_calc_Analytics{
    ...
    public function show_tracker_notice() {
        ...
        $notice .= '<p>'.__("Hi, your support can make a big difference!", 'product-page-shipping-calculator-woocommerce').'</p>';
        $notice .= '<p>'.__("We collect only technical data — including the plugin version, WordPress version, WooCommerce version, and site url — solely to improve compatibility and enhance plugin features.", 'product-page-shipping-calculator-woocommerce').'</p>';
        ...
        $notice .= sprintf(
            ...
            __('I Don\'t Help', 'product-page-shipping-calculator-woocommerce')
        );
        $notice .= sprintf(
            ...
            __('I Will Help', 'product-page-shipping-calculator-woocommerce')
        );
    ...
    public function handle_deactivation_form() {
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'pi_deactivate_nonce_' . $this->plugin_slug)) {
            wp_die(esc_html__('Security check failed', 'product-page-shipping-calculator-woocommerce'));
        }
        ...
        if (is_multisite() && is_plugin_active_for_network($this->plugin_path)) {
            if (is_super_admin()) {
                ...
                wp_die(esc_html__('You do not have permission to deactivate a network plugin.', 'product-page-shipping-calculator-woocommerce'));
            }
        } else {
...
new Pi_shipping_calc_Analytics(
    'WooCommerce Shipping Calculator',
    'product-page-shipping-calculator-woocommerce/pisol-product-page-shipping-calculator-woocommerce.php',
    PISOL_PRODUCT_PAGE_SHIPPING_CALCULATOR_WOOCOMMERCE_VERSION
);